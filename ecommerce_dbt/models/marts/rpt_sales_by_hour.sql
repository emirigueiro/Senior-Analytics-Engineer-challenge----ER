...
select
    ...
    coalesce(aggregated.units, 0)            s units,
    round(aggregated.revenue_usd / nullif(aggregated.orders_with_revenue, 0), 2)    as avg_order_value_usd,
    round(coalesce(aggregated.orders, 0) / sum(coalesce(aggregated.orders, 0)) over (), 4)                as share_of_orders
